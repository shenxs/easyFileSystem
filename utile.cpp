...
void init_dir() {
  ...
  touch(
      "/etc/passwd", "frwx------", 0,
      0); //只有root管理员用户组才可以查看,不保存字符串,直接用二进制保存用户信息,方便使用
...
int mkdir(string path, string permission, int userid, int groupid) {
  ...
  parent_node =
      getInode(readInode(0), parent_path.substr(1, parent_path.length()));
...
int showDir(Directory dir, vector<string> args) {
  ...
  if (canI(node, temp, 1)) {
      if(node.permissions[0]=='d')
          cout << "\033[0;35;1m" << dir.name << "\e[0m" << endl;
      else
          cout << "\033[0;31;1m" << dir.name << "\e[0m" << endl;
      return 1;
  } else {
      return 0;
  }
...
int showDirDetial(Directory dir, vector<string> args) {
    ...
    temp.group_id = group_id;
    if (canI(node, temp, 1)) {
        cout << node.permissions << " " << node.inode_id << " " << node.user_id
            << " " << node.group_id << " " << node.filesize << "B " << node.mtime
            << " " << dir.name << endl;
        return 1;
    }
    return 0;
