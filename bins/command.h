        ...
        if (canI(source_node, currentUser, 1)) {
          ...
          if (id == -1) {
            ...
            if (canI(target_parent_node, currentUser, 2)) {
              ...
              int copy_node_id = touch(path, "frw-rw-r--", currentUser.user_id,
            ...
            if (node.permissions[0] == 'd') {
              ...
              if (canI(node, currentUser, 2)) {
                ...
                if (path == "/")
                  ...
                  path = path + "/" + source_name;
                int copy_node_id =
                    touch(path, "frw-rw-r--", currentUser.user_id,
                          currentUser.group_id);
                Inode copynode = readInode(copy_node_id);
                string content = getfileContent(source_node);
                write2File(copynode, content);
      ...
      if (target.find_first_of("/") == string::npos) {
        ...
        if (id != -1) {
          ...
          if (canI(currentInode, currentUser, 2)) {
            int childid =
                touch(getPath(currentInode) + "/" + target, "lrwxrwxrwx",
                      currentUser.user_id, currentUser.group_id);
            currentInode = readInode(currentInode.inode_id);
          ...
          if (id != -1) {
            ...
            if (canI(node, currentUser, 2)) {
              ...
              if (path == "/")
                ...
                path = path + "/" + childname;
              int child_node_id = touch(path, "lrwxrwxrwx", currentUser.user_id,
                                        currentUser.group_id);
